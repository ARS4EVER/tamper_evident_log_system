"""默克尔树操作辅助函数。

采用严格的 RFC 6962 标准重新实现，确保包含证明与一致性证明拓扑一致。
"""

from typing import List
from ..crypto import DomainSeparatedHash


def _largest_power_of_two_less_than(n: int) -> int:
    """返回小于 n 的最大 2 的幂。"""
    if n <= 1:
        return 1
    return 1 << ((n - 1).bit_length() - 1)


def compute_root_from_leaves(leaves: List[bytes]) -> bytes:
    """从叶子节点列表计算默克尔树根 (RFC 6962 标准)。"""
    if not leaves:
        raise ValueError("叶子列表不能为空")
    if len(leaves) == 1:
        return leaves[0]
    
    k = _largest_power_of_two_less_than(len(leaves))
    left_root = compute_root_from_leaves(leaves[:k])
    right_root = compute_root_from_leaves(leaves[k:])
    return DomainSeparatedHash.hash_internal(left_root, right_root)


def get_tree_depth(tree_size: int) -> int:
    if tree_size <= 1:
        return 0
    return (tree_size - 1).bit_length()


def find_first_diff_level(old_size: int, new_size: int) -> int:
    xor = old_size ^ new_size
    if xor == 0:
        return 0
    return xor.bit_length() - 1


# ---------- 包含证明 ----------

def build_inclusion_proof(leaves: List[bytes], leaf_index: int) -> List[bytes]:
    """构建 RFC 6962 标准的包含证明。"""
    if leaf_index < 0 or leaf_index >= len(leaves):
        raise ValueError(f"无效的叶子索引: {leaf_index}")

    proof_hashes: List[bytes] = []

    def _build(sub_leaves: List[bytes], idx: int):
        if len(sub_leaves) == 1:
            return
        k = _largest_power_of_two_less_than(len(sub_leaves))
        if idx < k:
            proof_hashes.append(compute_root_from_leaves(sub_leaves[k:]))
            _build(sub_leaves[:k], idx)
        else:
            proof_hashes.append(compute_root_from_leaves(sub_leaves[:k]))
            _build(sub_leaves[k:], idx - k)

    _build(leaves, leaf_index)
    return proof_hashes[::-1]


def verify_inclusion_proof_with_hash(
    leaf_hash: bytes,
    proof_hashes: List[bytes],
    leaf_index: int,
    tree_size: int,
    root_hash: bytes,
) -> bool:
    """使用 RFC 6962 结构自底向上验证包含证明。"""
    if leaf_index < 0 or leaf_index >= tree_size:
        return False

    proof_iter = iter(proof_hashes)

    def _verify(index: int, size: int) -> bytes:
        if size == 1:
            return leaf_hash
        k = _largest_power_of_two_less_than(size)
        if index < k:
            left = _verify(index, k)
            right = next(proof_iter)
            return DomainSeparatedHash.hash_internal(left, right)
        else:
            left = next(proof_iter)
            right = _verify(index - k, size - k)
            return DomainSeparatedHash.hash_internal(left, right)

    try:
        computed_root = _verify(leaf_index, tree_size)
        try:
            next(proof_iter)
            return False  # 哈希有多余，证明非法
        except StopIteration:
            pass
        return computed_root == root_hash
    except StopIteration:
        return False


def verify_inclusion_proof_with_data(
    leaf_data: bytes,
    proof_hashes: List[bytes],
    leaf_index: int,
    tree_size: int,
    root_hash: bytes,
) -> bool:
    current_hash = DomainSeparatedHash.hash_leaf(leaf_data)
    return verify_inclusion_proof_with_hash(
        current_hash, proof_hashes, leaf_index, tree_size, root_hash
    )


# ---------- 一致性证明 ----------

def build_consistency_proof(
    leaves: List[bytes], old_size: int, new_size: int,
) -> List[bytes]:
    """基于 RFC 6962 算法构建一致性证明。"""
    if old_size <= 0 or new_size <= 0:
        raise ValueError("树大小必须为正")
    if old_size > new_size:
        raise ValueError("旧树大小不能大于新树大小")
    if old_size == new_size:
        return []

    def _subproof(m: int, sub_leaves: List[bytes], b: bool) -> List[bytes]:
        n = len(sub_leaves)
        if m == n:
            if b:
                return []
            return [compute_root_from_leaves(sub_leaves)]
        k = _largest_power_of_two_less_than(n)
        if m <= k:
            return _subproof(m, sub_leaves[:k], b) + [compute_root_from_leaves(sub_leaves[k:])]
        else:
            return _subproof(m - k, sub_leaves[k:], False) + [compute_root_from_leaves(sub_leaves[:k])]

    return _subproof(old_size, leaves[:new_size], True)


def verify_consistency_proof_rfc6962(
    old_root: bytes,
    new_root: bytes,
    proof_hashes: List[bytes],
    m: int,
    n: int,
) -> bool:
    """真正正确的 RFC 6962 验证算法 (自底向上追踪组合)"""
    if m < 0 or n < 0 or m > n:
        return False
    if m == n:
        return old_root == new_root and len(proof_hashes) == 0
    if m == 0:
        return len(proof_hashes) == 0

    fn = n - 1
    sn = m - 1
    while sn % 2 == 1:
        sn >>= 1
        fn >>= 1

    proof_iter = iter(proof_hashes)
    try:
        if sn == 0:
            fr = old_root
            sr = old_root
        else:
            fr = next(proof_iter)
            sr = fr

        while sn > 0:
            if sn % 2 == 1:
                sibling = next(proof_iter)
                fr = DomainSeparatedHash.hash_internal(sibling, fr)
                sr = DomainSeparatedHash.hash_internal(sibling, sr)
            else:
                sibling = next(proof_iter)
                fr = DomainSeparatedHash.hash_internal(fr, sibling)
            sn >>= 1
            fn >>= 1

        while fn > 0:
            sibling = next(proof_iter)
            fr = DomainSeparatedHash.hash_internal(fr, sibling)
            fn >>= 1

        try:
            next(proof_iter)
            return False  # 有多余哈希残余
        except StopIteration:
            pass

        return fr == new_root and sr == old_root

    except StopIteration:
        return False


def verify_consistency_proof_by_recomputation(
    all_leaves: List[bytes], old_root: bytes, new_root: bytes, m: int, n: int,
) -> bool:
    if m < 0 or n < 0 or m > n or n > len(all_leaves):
        return False
    if m == 0:
        return True
    if compute_root_from_leaves(all_leaves[:m]) != old_root:
        return False
    return compute_root_from_leaves(all_leaves[:n]) == new_root